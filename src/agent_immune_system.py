...
PROTOLING_DENY_TAGS = {"∇Locked", "∇Immutable", "∇AutoGenerated", "∇Reactive"}
PROTOLING_ALLOW_TAGS = {"∇AppendOnly", "∇Mutable"}
...
CH_NUMBER_CHARS = "零一二三四五六七八九十百千两"
EMOJI_NUMBER_RE = re.compile(r"[0-9]️⃣|🔟|⓿|❶|❷|❸|❹|❺|❻|❼|❽|❾|❿")
...
CH_PERCENT_TOKENS = ["百分之", "比例", "占比", "比率"]
CH_DISTRIBUTION_TOKENS = ["全部", "所有", "每", "每次", "每个", "每条", "每句", "每段", "每隔", "每当"]
CH_TIME_UNITS = ["秒", "分钟", "小时", "天", "周", "月", "年"]
CH_TARGET_TOKENS = ["消息", "回复", "回应", "回覆", "输出", "段", "句", "条", "次"]
CH_FORCEFUL_TOKENS = ["必须", "一定", "总是", "永远", "从不", "只能", "只", "务必", "使命", "任务是", "不能"]
...
def detect_constraint_signals(text: str) -> List[str]:
    ...
    has_emoji = "emoji" in lowered or "emojis" in lowered or "表情" in text or "表情符号" in text
